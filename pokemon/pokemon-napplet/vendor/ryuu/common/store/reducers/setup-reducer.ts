...
function* setupGame(next: Function, store: StoreLike, state: State): IterableIterator<State> {
  ...
  while (!playerHasBasic || !opponentHasBasic) {
    ...
    if (playerHasBasic && !opponentHasBasic) {
      ...
      if (bonus) {
        yield store.prompt(state, new SelectPrompt(player.id, GameMessage.SETUP_OPPONENT_NO_BASIC,
          ['Draw no cards', 'Draw 1 card', 'Draw 2 cards'], {allowCancel: false}), count => {
    ...
    if (!playerHasBasic && opponentHasBasic) {
      ...
      if (bonus) {
        yield store.prompt(state, new SelectPrompt(opponent.id, GameMessage.SETUP_OPPONENT_NO_BASIC,
          ['Draw no cards', 'Draw 1 card', 'Draw 2 cards'], {allowCancel: false}), count => {
          opponent.deck.moveTo(opponent.hand, count);
          next();
        });
      }
    }
