...
function* useAttack(next: Function, store: StoreLike, state: State, effect: UseAttackEffect): IterableIterator<State> {
  ...
  if (sp.includes(SpecialCondition.CONFUSED)) {
    ...
    if (flip === false) {
      ...
      store.reduceEffect(state, stats);
      const types = new CheckPokemonTypeEffect(player.active);
      store.reduceEffect(state, types);
      player.active.damage += Math.max(0, applyWeaknessAndResistance(20, types.cardTypes, stats.weakness, stats.resistance));
