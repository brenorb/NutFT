...
function retreatPokemon(store: StoreLike, state: State, effect: RetreatEffect) {
  ...
  const retreat = () => player.switchPokemon(player.bench[effect.benchIndex]);
  // Base Set confusion: pay retreat cost first, then flip; tails stays Active.
  if (player.active.specialConditions.includes(SpecialCondition.CONFUSED)) {
    store.prompt(state, new CoinFlipPrompt(player.id, GameMessage.FLIP_CONFUSION), result => { if (result) retreat(); });
  } else retreat();
}
